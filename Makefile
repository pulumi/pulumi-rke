...
prepare::
	...
	if [[ "${OS}" != "Darwin" ]]; then \
		sed -i 's,github.com/pulumi/pulumi-rke,${REPOSITORY},g' go.mod; \
...
tfgen::
	go install -ldflags "-X github.com/jaxxstorm/pulumi-${PACK}/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${TFGEN}

provider::
	go install -ldflags "-X github.com/jaxxstorm/pulumi-${PACK}/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${PROVIDER}

lint::
	golangci-lint run

install::
	GOBIN=$(PULUMI_BIN) go install -ldflags "-X github.com/jaxxstorm/pulumi-${PACK}/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${PROVIDER}
